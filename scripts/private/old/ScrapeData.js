...
agent.addListener('next', function (error, agent) {
  ...
  jsdom.jQueryify(window, 'http://code.jquery.com/jquery-1.4.2.min.js', function (window, jquery) {
    ...
    if (phones.length) {
      ...
      (phone.availability = c1
        .find('table:nth-child(1) th:contains("Availability")+td')
        .text()
        .trim()
        .split(/\s*\n\s*/)),
        (phone.battery = {
      ...
      phone.sizeAndWeight = {
        dimensions: c2
          .find('table:nth-child(2) th:contains("Dimensions")+td')
          .text()
          .trim()
          .split(/\s*\n\s*/),
      ...
      phone.display = {
        ...
        touchScreen: boolean(
          c2.find('table:nth-child(3) th:contains("Touch screen")+td img').attr('src')
        )
      };
      phone.hardware = {
        fmRadio: boolean(c2.find('table:nth-child(4) th:contains("FM Radio")+td img').attr('src')),
        physicalKeyboard: c2
          .find('table:nth-child(4) th:contains("Physical keyboard")+td img')
          .attr('src'),
        accelerometer: boolean(
          c2.find('table:nth-child(4) th:contains("Accelerometer")+td img').attr('src')
        ),
      ...
      phone.camera = {
        ...
        features: c2
          .find('table:nth-child(5) th:contains("Features")+td')
          .text()
          .trim()
          .split(/\s*\n\s*/)
      };
      ...
      body.find('ul.phonelist li.list').each(function (a) {
        ...
        phone.age = age++;
        phone.imageUrl = 'http://google.com' + jquery(this).find('img.phone').attr('src');
